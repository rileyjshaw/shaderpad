...
const siteOrigin = 'https://rileyjshaw.com';
...
function compareEntries(a, b) {
  const groupDiff =
    markdownGroupOrder.indexOf(a.group) - markdownGroupOrder.indexOf(b.group);
  if (groupDiff !== 0) {
    return groupDiff;
  }
...
function renderMarkdownIndex(entries) {
  ...
  for (const group of markdownGroupOrder) {
    ...
    for (const entry of groupEntries) {
      lines.push(
        `- [${entry.title}](${entry.markdownUrl})${entry.description ? ` - ${entry.description}` : ''}`,
      );
    }
    lines.push('');
  }
...
function renderExampleIndex(entries) {
  ...
  for (const entry of entries) {
    lines.push(
      `- [${entry.title}](${entry.assetUrl})${entry.description ? ` - ${entry.description}` : ''}`,
    );
  }
...
function sanitizeMarkdocForLlmsFull(body, shaderpadSizeValue) {
  ...
  output = output.replace(
    /{%\s*quick-links\s*%}([\s\S]*?){%\s*\/quick-links\s*%}/g,
    (_, inner) => renderQuickLinks(inner),
  );
  output = output.replace(
...
const shaderpadSizeValue =
  /shaderpadStandardImportGzip = "([^"]+)";/.exec(generatedSizeSource)?.[1] ??
  '6.1 kB';
...
const repoReadmeEntry = {
  ...
  htmlUrl: null,
  sourceUrl: 'https://github.com/rileyjshaw/shaderpad/blob/main/README.md',
};
...
const llmsOutput = llmsTemplate
  ...
  .replace(
    ...
    renderAssetIndex(
      curatedExampleTitles
        .map(title => exampleEntries.find(entry => entry.title === title))
        .filter(Boolean),
    ),
  )
  .replace(
    '{{MARKDOWN_DOCS_INDEX}}',
    renderMarkdownIndex([...docEntries, repoReadmeEntry].sort(compareEntries)),
  );
