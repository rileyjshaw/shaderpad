...
| -------------- | ------- | --------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `u_time`       | `float` | `0.0`                       | Updated by `play()` and `step()` before each rendered frame                   | ShaderPad playback time in seconds. `pause()` preserves it, `rewind()` resets it, `draw()` does not advance it, and `updateUniforms({ u_time })` sets the next timing baseline. |
| `u_frame`      | `int`   | `0`                         | Updated by `play()` and `step()` before each rendered frame                   | Current frame index. `draw()` does not advance it, and `updateUniforms({ u_frame })` sets the next frame baseline.                                                              |
