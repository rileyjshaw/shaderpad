...
| -------------- | ------- | --------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `u_time`       | `float` | `0.0`                       | Updated by `play()` and `step()` before each rendered frame                   | ShaderPad playback time in seconds. `pause()` preserves it, `rewind()` resets it, and `draw()` does not advance it. |
