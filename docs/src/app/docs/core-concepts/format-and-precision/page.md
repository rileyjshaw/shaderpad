...
- The intermediate render texture
...
Due to browser limitations, constructor texture options cannot increase the color precision of the displayed canvas beyond its native 8-bit. Reduced-channel formats such as `R8` or `RG8` can visibly change the result by dropping color channels.
...
- Normalized color formats such as `R8` or `RGBA8`, and float formats such as `R32F` or `RGBA32F`, use `sampler2D`
- Unsigned integer formats such as `R8UI` or `RGBA16UI`, use `usampler2D`
- Signed integer formats such as `R32I` or `RGBA8I`, use `isampler2D`

The same rule applies to history textures, which are stored as `sampler2DArray`, `usampler2DArray`, or `isampler2DArray`.
...
If you omit `format`, ShaderPad derives one from `internalFormat`. The defaults are practical, but if you care about channel count or integer pipelines, set all three options explicitly.
