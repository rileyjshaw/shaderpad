...
function syncFavicon(theme: string) {
	let icon = theme === 'dark' ? '/shaderpad/icon-dark.svg' : '/shaderpad/icon-light.svg';
	let links = Array.from(document.head.querySelectorAll<HTMLLinkElement>('link[rel~="icon"]'));
		...
		let observer = new MutationObserver(mutations => {
			let iconLinkChanged = mutations.some(({ addedNodes, removedNodes }) =>
				[...addedNodes, ...removedNodes].some(
					node => node instanceof HTMLLinkElement && node.relList.contains('icon'),
				),
			);
