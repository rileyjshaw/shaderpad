...
function createPlaybackVisibilityController({ target, autoplay, autopause, isPlaying, play, pause, onVisibilityChange }) {
	...
	const sync = () => {
		...
		if (!currentAutoplay) {
			if (isManagedPlayback && isPlaying()) pause();
...
function parseTextureOptionsFromAttributes(readAttribute, prefix = "") {
	...
	if (preserveYValue != null) options.preserveY = parseBooleanLikeValue(preserveYValue, true);
...
async function loadDomTextureSource(element) {
	...
	if (element.width <= 0 || element.height <= 0) throw new Error("Texture canvas elements must have a positive width and height.");
	return element;
}
function getLiveDomTextureSource(element) {
	if (element instanceof HTMLVideoElement) return element.videoWidth > 0 && element.videoHeight > 0 ? element : void 0;
	if (element instanceof HTMLCanvasElement) return element.width > 0 && element.height > 0 ? element : void 0;
}
...
//# sourceMappingURL=declarative-textures-CLw4roIN.mjs.map