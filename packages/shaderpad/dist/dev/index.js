...
var ShaderPad = class ShaderPad {
	...
	constructor(fragmentShaderSrc, { canvas, plugins, history, cursorTarget, ...texOptions } = {}) {
		...
		if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
			const linkError = require_util.spError(2, {
	...
	reserveTex(name) {
		...
		if (this.texPool.next >= this.texPool.max) throw require_util.spError(5, {
	...
	resolveTexOpts(options) {
		...
		if ((result.internalFormat === gl.RGBA16F || result.internalFormat === gl.RGBA32F) && !gl.getExtension("EXT_color_buffer_float")) throw require_util.spError(6, {
	...
	_updateUniforms(updates, options) {
		...
		Object.entries(updates).forEach(([name, newValue]) => {
			...
			if (uniform.arrayLength) {
				...
				if (!nValues) return;
				if (nValues > uniform.arrayLength) throw require_util.spError(11, {
					name,
					receivedLength: nValues,
					maxLength: uniform.arrayLength
				});
				if (newValue.some((item) => (Array.isArray(item) ? item.length : 1) !== uniform.length)) throw require_util.spError(12, {
					name,
				...
				if (options?.startIndex) {
					...
					if (!newLocation) throw require_util.spError(13, {
						name,
				...
				if (scalarValue.length !== uniform.length) throw require_util.spError(14, {
					name,
	...
	initTex(name, source, options) {
		...
		if (!width || !height) throw require_util.spError(17, {
			name: stringFrom(name),
			width,
			height,
	...
	updateTex(name, source, historySlots) {
		...
		if (source instanceof ShaderPad) {
			...
			if (source.gl === gl) {
				...
				const targetSlots = historySlots === void 0 ? [info.history.writeIndex] : Array.isArray(historySlots) ? historySlots.map((i) => require_util.safeMod(i, depth)) : [require_util.safeMod(historySlots, depth)];
				gl.activeTexture(gl.TEXTURE0 + info.unitIndex);
