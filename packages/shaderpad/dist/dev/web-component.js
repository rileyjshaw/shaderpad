...
const ShaderPadElementBase = typeof HTMLElement === "undefined" ? class {} : HTMLElement;
...
function ensureShaderPadHostDefaults(element) {
	if (!element.hasAttribute(HOST_DEFAULT_ATTRIBUTE)) element.setAttribute(HOST_DEFAULT_ATTRIBUTE, "");
	const document = element.ownerDocument;
	if (!document || document.getElementById(HOST_DEFAULT_STYLE_ID)) return;
	const style = document.createElement("style");
	style.id = HOST_DEFAULT_STYLE_ID;
	style.textContent = [`:where([${HOST_DEFAULT_ATTRIBUTE}]) { display: block; width: 100%; }`, `:where([${HOST_DEFAULT_ATTRIBUTE}]:not([for])) { height: 100%; }`].join("\n");
...
var ShaderPadElement = class ShaderPadElement extends ShaderPadElementBase {
	...
	constructor() {
		...
		this.autoplayValue = require_declarative_textures.parseBooleanLikeValue(this.getAttribute("autoplay"), defaults.autoplay);
		this.autopauseValue = require_declarative_textures.parseBooleanLikeValue(this.getAttribute("autopause"), defaults.autopause);
	}
	get shader() {
	...
	attributeChangedCallback(name, oldValue, newValue) {
		...
		const defaults = this.constructor.shaderPadConfig;
		const value = require_declarative_textures.parseBooleanLikeValue(newValue, defaults[name]);
	...
	ensureInitialized() {
		...
		if (!this.isConnected) return Promise.reject(/* @__PURE__ */ new Error("Cannot initialize <shader-pad> while it is disconnected."));
	...
	async initialize() {
		try {
			...
			if (!fragmentScript) throw new Error("A direct child <script type=\"x-shader/x-fragment\"> is required.");
	...
	setupPlayback(instance, canvas) {
		...
		this.playbackController = require_declarative_textures.createPlaybackVisibilityController({
			target: canvas,
	...
	getFragmentShaderScript() {
		return Array.from(this.children).find((child) => child instanceof HTMLScriptElement && child.getAttribute("type") === "x-shader/x-fragment");
	}
	...
	resolveRenderCanvas() {
		...
		if (forId) {
			...
			if (!(candidate instanceof HTMLCanvasElement)) throw new Error(`No <canvas> with id "${forId}" was found for <shader-pad for="${forId}">.`);
			return {
				canvas: candidate,
				owned: false
			};
		}
		const ownedCanvas = Array.from(this.children).find((child) => child instanceof HTMLCanvasElement && child !== this.generatedCanvas && !child.hasAttribute("data-texture"));
		if (ownedCanvas) {
			this.removeGeneratedCanvas();
			return {
				canvas: ownedCanvas,
		...
		return {
			canvas: this.ensureGeneratedCanvas(),
			owned: true
		};
	}
	...
	removeGeneratedCanvas() {
		if (this.generatedCanvas?.parentElement === this) this.generatedCanvas.remove();
	}
	getTextureBindings() {
		const bindings = [];
		for (const child of Array.from(this.children)) {
			...
			if (!name) continue;
			if (require_declarative_textures.isDomTextureElement(child)) {
				bindings.push({
					element: child,
					name,
			...
			if (child instanceof ShaderPadElement) bindings.push({
				element: child,
				name,
				options: require_declarative_textures.parseTextureOptions(child, "data-texture-"),
				live: true
	...
	installChildListeners(bindings) {
		for (const binding of bindings) {
			if (require_declarative_textures.isDomTextureElement(binding.element)) {
	...
	showControlChildren() {
		for (const [element, previousHidden] of this.controlVisibility) element.hidden = previousHidden;
		this.controlVisibility.clear();
	}
	updateLiveTextures(nestedRenderMode) {
		...
		for (const binding of this.liveTextures) {
			...
			if (binding.element instanceof ShaderPadElement && binding.element.shader) {
				if (nestedRenderMode === "step") binding.element.step();
				else if (nestedRenderMode === "draw") binding.element.draw();
	...
	pauseManagedTextures() {
		for (const binding of this.liveTextures) if (binding.element instanceof ShaderPadElement) binding.element.pause();
	}
	...
	dispatchMutablePreStep(instance, time, frame, options) {
		...
		if (options && detail.stepOptions) Object.assign(options, detail.stepOptions);
	}
};
...
//#endregion
exports.ShaderPadElement = ShaderPadElement;
exports.createShaderPadElement = createShaderPadElement;
