...
const LANDMARK_INDICES = {
	...
	LEFT_HAND_CENTER: 34,
	RIGHT_HAND_CENTER: 35,
	LEFT_FOOT_CENTER: 36,
	RIGHT_FOOT_CENTER: 37,
	TORSO_CENTER: 38
};
...
function updateLandmarksData(detector, poses) {
	...
	for (let poseIdx = 0; poseIdx < nPoses; ++poseIdx) {
		...
		const bodyCenter = require_plugins_mediapipe_common.calculateBoundingBoxCenter(data, poseIdx, ALL_STANDARD_INDICES, LANDMARK_COUNT, N_LANDMARK_METADATA_SLOTS);
		...
		const leftHandCenter = require_plugins_mediapipe_common.calculateBoundingBoxCenter(data, poseIdx, LEFT_HAND_INDICES, LANDMARK_COUNT, N_LANDMARK_METADATA_SLOTS);
		...
		const rightHandCenter = require_plugins_mediapipe_common.calculateBoundingBoxCenter(data, poseIdx, RIGHT_HAND_INDICES, LANDMARK_COUNT, N_LANDMARK_METADATA_SLOTS);
		...
		const leftFootCenter = require_plugins_mediapipe_common.calculateBoundingBoxCenter(data, poseIdx, LEFT_FOOT_INDICES, LANDMARK_COUNT, N_LANDMARK_METADATA_SLOTS);
		...
		const rightFootCenter = require_plugins_mediapipe_common.calculateBoundingBoxCenter(data, poseIdx, RIGHT_FOOT_INDICES, LANDMARK_COUNT, N_LANDMARK_METADATA_SLOTS);
		...
		const torsoCenter = require_plugins_mediapipe_common.calculateBoundingBoxCenter(data, poseIdx, TORSO_INDICES, LANDMARK_COUNT, N_LANDMARK_METADATA_SLOTS);
...
function updateMask(detector, segmentationMasks) {
	...
	if (!segmentationMasks || segmentationMasks.length === 0) return maskShader.clear();
	...
	return function(shaderPad, context) {
		...
		async function initializeDetector() {
			detector = await require_plugins_mediapipe_common.getOrCreateSharedResource(optionsKey, sharedDetectors, sharedDetectorPromises, async () => {
				const [mediaPipe, { PoseLandmarker }] = await Promise.all([require_plugins_mediapipe_common.getSharedFileset(wasmBaseUrl), import("@mediapipe/tasks-vision")]);
				if (destroyed) return;
		...
		shaderPad.on("_init", () => {
			...
			shaderPad.initializeTexture("u_poseLandmarksTex", {
				data: landmarksData,
				width: LANDMARKS_TEXTURE_WIDTH,
		...
		shaderPad.on("initializeTexture", (name, source) => {
			if (name === textureName && require_plugins_mediapipe_common.isMediaPipeSource(source)) requestPoses(source);
		});
		shaderPad.on("updateTextures", (updates) => {
			const source = updates[textureName];
			if (require_plugins_mediapipe_common.isMediaPipeSource(source)) requestPoses(source);
		});
...
${fn("vec4", "poseLandmark", "int poseIndex, int landmarkIndex", `int i = ${N_LANDMARK_METADATA_SLOTS} + poseIndex * ${LANDMARK_COUNT} + landmarkIndex;
...
${fn("vec2", "poseAt", "vec2 pos", `${sampleMask}
