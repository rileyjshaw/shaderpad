...
const FACE_LANDMARK_MOUTH_CENTER = 479;
let innerMouthIndices = null;
...
function normalizeChannelBitValues(bitValues) {
	return Object.fromEntries(Object.entries(bitValues).map(([name, bitValue]) => [name, bitValue / CHANNEL_BIT_SCALE]));
}
...
function stripTriangulate(a, b) {
	...
	for (let i = 0; i < n - 1; ++i) tris.push(a[i], b[i], b[i + 1], a[i], b[i + 1], a[i + 1]);
	return tris;
}
...
function initFaceRegions(LandmarkerClass) {
	if (!faceRegions) {
		...
		faceRegions = Object.fromEntries(Object.entries({
			...
			indices,
			vertices: new Float32Array(indices.length * 2)
		}]));
	}
}
...
function initMaskRenderer(canvas) {
	...
	if (!gl) throw require_util.spError(60, {
	...
	try {
		...
		if (!regionPositionBuffer || regionPositionLocation < 0 || !quadBuffer || blitPositionLocation < 0 || !colorLocation || !textureLocation || !scratchTexture || !scratchFramebuffer || framebufferStatus !== gl.FRAMEBUFFER_COMPLETE) throw new Error();
		...
		throw require_util.spError(62, { framebufferStatus });
	}
}
...
function updateLandmarksData(detector, faces) {
	...
	for (let faceIdx = 0; faceIdx < nFaces; ++faceIdx) {
		...
		const faceCenter = require_plugins_mediapipe_common.calculateBoundingBoxCenter(data, faceIdx, ALL_STANDARD_INDICES, LANDMARK_COUNT, N_LANDMARK_METADATA_SLOTS);
		data.set(faceCenter, (N_LANDMARK_METADATA_SLOTS + faceIdx * LANDMARK_COUNT + FACE_LANDMARK_FACE_CENTER) * 4);
		const mouthCenter = require_plugins_mediapipe_common.calculateBoundingBoxCenter(data, faceIdx, innerMouthIndices, LANDMARK_COUNT, 1);
...
function updateMask(detector, width, height) {
	...
	for (let faceIdx = 0; faceIdx < nFaces; ++faceIdx) {
		...
		accumulateScratch(mask);
		drawRegionToScratch(mask, landmarksData, faceRegions.LEFT_EYEBROW, faceIdx, RED_CHANNEL_VALUES.LEFT_EYEBROW, 0, 0);
		accumulateScratch(mask);
		drawRegionToScratch(mask, landmarksData, faceRegions.RIGHT_EYEBROW, faceIdx, RED_CHANNEL_VALUES.RIGHT_EYEBROW, 0, 0);
		accumulateScratch(mask);
		...
		accumulateScratch(mask);
		drawRegionToScratch(mask, landmarksData, faceRegions.INNER_MOUTH, faceIdx, RED_CHANNEL_VALUES.INNER_MOUTH, 0, 0);
		accumulateScratch(mask);
	}
}
function face(config) {
	const { textureName, wasmBaseUrl = require_plugins_mediapipe_common.DEFAULT_WASM_BASE_URL, options: { history, ...mediapipeOptions } = {} } = config;
	const options = {
		...DEFAULT_FACE_OPTIONS,
		...mediapipeOptions
	};
	const optionsKey = require_plugins_mediapipe_common.hashOptions({
		...options,
		textureName,
		wasmBaseUrl
	});
	...
	return function(shaderPad, context) {
		...
		function onResult() {
			...
			} else writeTextures(historySlot);
			emit("face:result", detector.state.result);
		}
		function notifySubscribers() {
			if (destroyed || !detector) return;
			const subscribers = detector.subscribers;
			for (const [cb, needsResult] of subscribers) if (needsResult) {
				cb();
				if (destroyed) return;
		...
		async function initializeDetector() {
			detector = await require_plugins_mediapipe_common.getOrCreateSharedResource(optionsKey, sharedDetectors, sharedDetectorPromises, async () => {
				const [mediaPipe, { FaceLandmarker }] = await Promise.all([require_plugins_mediapipe_common.getSharedFileset(wasmBaseUrl), import("@mediapipe/tasks-vision")]);
				if (destroyed) return;
		...
		async function detectFaces(source) {
			...
			detector.state.pending = detector.state.pending.then(async () => {
				...
				if (source !== detector.state.source) {
					...
					if (now - detector.state.resultTimestamp > 2) shouldDetect = true;
				}
				if (shouldDetect) {
					let result;
					let width, height;
				...
				} else if (detector.state.result) notifySubscribers();
			});
		...
		shaderPad.on("_init", () => {
			...
			shaderPad.initializeTexture("u_faceLandmarksTex", {
				data: landmarksData,
				width: LANDMARKS_TEXTURE_WIDTH,
		...
		shaderPad.on("initializeTexture", (name, source) => {
			if (name === textureName && require_plugins_mediapipe_common.isMediaPipeSource(source)) requestFaces(source);
		});
		shaderPad.on("updateTextures", (updates) => {
			const source = updates[textureName];
			if (require_plugins_mediapipe_common.isMediaPipeSource(source)) requestFaces(source);
		});
		...
		const { fn, historyParams } = require_plugins_mediapipe_common.generateGLSLFn(history);
		const sampleMask = history ? `_sampleFaceMask(pos, framesAgo)` : `texture(u_faceMask, pos)`;
		const decodeFaceBitIndex = Array.from({ length: GB_BITMASK_MAX_FACES - 1 }, (_, i) => `step(${2 ** (i + 1)}.0, faceBitF)`).join(" + ");
	...
	float faceIndex = ${decodeFaceBitIndex} - (1.0 - hasFace);` : `float faceIndex = float(int(uint(mask.b * ${CHANNEL_BIT_SCALE}.0 + 0.5)) - 1);`;
		const checkAt = (fnName, ...regionNames) => fn("vec2", `${fnName}At`, "vec2 pos", `vec4 mask = ${sampleMask};
	${decodeFaceIndex}
	uint bits = uint(mask.r * ${CHANNEL_BIT_SCALE}.0 + 0.5);
	float hit = sign(float(bits & ${regionNames.reduce((mask, regionName) => mask | RED_REGION_BIT_VALUES[regionName], 0)}u));
	return vec2(hit, mix(-1.0, faceIndex, hit));`);
		const combineLeftRight = (fnName, leftFn, rightFn) => fn("vec2", `${fnName}At`, "vec2 pos", `vec2 left = ${leftFn}(pos${historyParams});
	vec2 right = ${rightFn}(pos${historyParams});
	return mix(right, left, left.x);`);
		const checkIn = (fnNames) => fnNames.map((fnName) => fn("float", `in${fnName[0].toUpperCase() + fnName.slice(1)}`, "vec2 pos", `vec2 a = ${fnName}At(pos${historyParams}); return step(0.0, a.y) * a.x;`)).join("\n");
		injectGLSL(`
...
${fn("vec4", "faceLandmark", "int faceIndex, int landmarkIndex", `int i = ${N_LANDMARK_METADATA_SLOTS} + faceIndex * ${LANDMARK_COUNT} + landmarkIndex;
...
${fn("vec2", "faceAt", "vec2 pos", `vec4 mask = ${sampleMask};
	${decodeFaceIndex}
