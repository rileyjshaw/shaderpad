...
const LANDMARK_COUNT = 22;
const HAND_CENTER_LANDMARKS = [
	0,
	0,
	5,
	9,
...
function updateLandmarksData(detector, hands, handedness) {
	...
	for (let handIdx = 0; handIdx < nHands; ++handIdx) {
		...
		const handCenter = require_plugins_mediapipe_common.calculateBoundingBoxCenter(data, handIdx, HAND_CENTER_LANDMARKS, LANDMARK_COUNT, N_LANDMARK_METADATA_SLOTS);
...
function hands(config) {
	...
	return function(shaderPad, context) {
		...
		shaderPad.on("_init", () => {
			...
			shaderPad.initializeTexture("u_handLandmarksTex", {
				data: landmarksData,
				width: LANDMARKS_TEXTURE_WIDTH,
		...
		shaderPad.on("initializeTexture", (name, source) => {
			if (name === textureName && require_plugins_mediapipe_common.isMediaPipeSource(source)) requestHands(source);
		});
		shaderPad.on("updateTextures", (updates) => {
			const source = updates[textureName];
			if (require_plugins_mediapipe_common.isMediaPipeSource(source)) requestHands(source);
		});
...
${fn("vec4", "handLandmark", "int handIndex, int landmarkIndex", `int i = ${N_LANDMARK_METADATA_SLOTS} + handIndex * ${LANDMARK_COUNT} + landmarkIndex;
