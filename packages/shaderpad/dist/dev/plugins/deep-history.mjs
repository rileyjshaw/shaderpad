import { r as spError, t as getSourceDimensions } from "../util-aX4PUebh.mjs";
...
function deepHistory(accessorName, initialSource, { history, chunks = 2, ...textureOptions }) {
	...
	if (!/^[_a-zA-Z]\w*$/.test(accessorName) || !Number.isInteger(history) || history < 1 || !Number.isInteger(chunks) || chunks < 1 || chunks > capacity) throw spError(64, {
		name: accessorName,
		history,
		chunks
	});
	...
	if (!isOutputHistory && (!width || !height)) throw spError(17, {
		name: accessorName,
		width,
		height,
	...
	const update = (source, historySlots) => {
		if (!active || !updateTexture || !shaderPad) throw spError(65, {
			name: accessorName,
			operation: "update"
		});
	...
	const plugin = (shader, context) => {
		...
		if (physicalDepth > maxArrayTextureLayers) throw spError(64, {
			name: accessorName,
			history,
			chunks,
			physicalDepth,
...
//#endregion
export { SHADER_OUTPUT, deepHistory as default };
//# sourceMappingURL=deep-history.mjs.map