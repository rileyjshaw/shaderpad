...
function updateLandmarksData(detector, faces) {
	...
	for (let faceIdx = 0; faceIdx < nFaces; ++faceIdx) {
		...
		const faceCenter = calculateBoundingBoxCenter(data, faceIdx, ALL_STANDARD_INDICES, LANDMARK_COUNT, N_LANDMARK_METADATA_SLOTS);
...
function face(config) {
	...
	return function(shaderPad, context) {
		...
		async function initializeDetector() {
			detector = await getOrCreateSharedResource(optionsKey, sharedDetectors, sharedDetectorPromises, async () => {
				const [mediaPipe, { FaceLandmarker }] = await Promise.all([getSharedFileset(wasmBaseUrl), import("@mediapipe/tasks-vision")]);
				if (destroyed) return;
		...
		shaderPad.on("initializeTexture", (name, source) => {
			if (name === textureName && isMediaPipeSource(source)) requestFaces(source);
		});
		shaderPad.on("updateTextures", (updates) => {
			const source = updates[textureName];
			if (isMediaPipeSource(source)) requestFaces(source);
		});
