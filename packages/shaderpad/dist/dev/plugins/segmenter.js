...
function updateMask(detector, categoryMask, confidenceMasks) {
	const { outputConfidenceMasks, mask: { shader, confidence } } = detector;
...
function segmenter(config) {
	...
	return function(shaderPad, context) {
		...
		const mediapipeCanvas = sharedDetectors.get(optionsKey)?.mask.canvas ?? new OffscreenCanvas(1, 1);
		let detector;
		let destroyed = false;
		let historySlot = -1;
		...
		async function initializeDetector() {
			detector = await require_plugins_mediapipe_common.getOrCreateSharedResource(optionsKey, sharedDetectors, sharedDetectorPromises, async () => {
				const [mediaPipe, { ImageSegmenter }] = await Promise.all([require_plugins_mediapipe_common.getSharedFileset(wasmBaseUrl), import("@mediapipe/tasks-vision")]);
				if (destroyed) return;
		...
		shaderPad.on("initializeTexture", (name, source) => {
			if (name === textureName && require_plugins_mediapipe_common.isMediaPipeSource(source)) requestSegments(source);
		});
		shaderPad.on("updateTextures", (updates) => {
			const source = updates[textureName];
			if (require_plugins_mediapipe_common.isMediaPipeSource(source)) requestSegments(source);
		});
			...
			detector.state.pending = detector.state.pending.then(async () => {
				...
				if (shouldDetect) {
					...
					if (result) {
						...
						if (result.categoryMask) updateMask(detector, result.categoryMask, result.confidenceMasks);
						notifySubscribers();
					}
		...
		const { fn } = require_plugins_mediapipe_common.generateGLSLFn(history);
...
${fn("vec2", "segmentAt", "vec2 pos", `${sampleMask}
