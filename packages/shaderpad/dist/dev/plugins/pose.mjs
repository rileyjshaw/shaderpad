...
function pose(config) {
	...
	return function(shaderPad, context) {
		...
		const maskShader = existingDetector?.maskShader ?? (() => {
			const shader = new ShaderPad(MASK_SHADER_SRC, { canvas: new OffscreenCanvas(1, 1) });
		...
		async function initializeDetector() {
			detector = await getOrCreateSharedResource(optionsKey, sharedDetectors, sharedDetectorPromises, async () => {
				const [mediaPipe, { PoseLandmarker }] = await Promise.all([getSharedFileset(wasmBaseUrl), import("@mediapipe/tasks-vision")]);
				if (destroyed) return;
		...
		shaderPad.on("initializeTexture", (name, source) => {
			if (name === textureName && isMediaPipeSource(source)) requestPoses(source);
		});
		shaderPad.on("updateTextures", (updates) => {
			const source = updates[textureName];
			if (isMediaPipeSource(source)) requestPoses(source);
		});
