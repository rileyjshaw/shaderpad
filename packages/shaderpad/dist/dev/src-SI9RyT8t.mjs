import { n as safeMod, r as spError, t as getSourceDimensions } from "./util-aX4PUebh.mjs";
...
var ShaderPad = class ShaderPad {
	...
	constructor(fragmentShaderSrc, options = {}) {
		...
		if (renderFormat?.includes("F") && !gl.getExtension("EXT_color_buffer_float")) throw spError(6, {
			format: renderFormat,
	...
	#reserveTexture(name) {
		...
		if (this.#texturePool.next_ >= this.#texturePool.max_) throw spError(5, {
	...
	#updateUniforms(updates, options) {
		...
		Object.entries(updates).forEach(([name, newValue]) => {
			...
			if (uniform.arrayLength_) {
				...
				if (!nValues) return;
				if (nValues > uniform.arrayLength_) throw spError(11, {
					name,
					receivedLength: nValues,
					maxLength: uniform.arrayLength_
				});
				if (newValue.some((item) => (Array.isArray(item) ? item.length : 1) !== uniform.length_)) throw spError(12, {
					name,
					expectedElementLength: uniform.length_
				});
				if ((options?.startIndex ?? 0) + nValues > uniform.arrayLength_) throw spError(11, {
					name,
				...
				if (scalarValue.length !== uniform.length_) throw spError(14, {
					name,
	...
	#initializeTexture(name, source, options) {
		...
		if (!width_ || !height_) throw spError(17, {
			name: stringFrom(name),
			width_,
			height_,
	...
	#updateTexture(name, source, historySlots) {
		...
		if (source instanceof ShaderPad) {
			...
			if (source.gl === gl) {
				...
				const targetSlots = historySlots === void 0 ? [info.history_.writeIndex_] : (Array.isArray(historySlots) ? historySlots : [historySlots]).map((slot) => safeMod(slot, depth_));
				gl.activeTexture(gl.TEXTURE0 + info.unitIndex_);
...
//#endregion
export { ShaderPad as t };
//# sourceMappingURL=src-SI9RyT8t.mjs.map