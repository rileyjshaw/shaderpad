...
}`,y=new Map,ae=new Map;function de(I,_){let e=I.landmarks.data,O=_.length;e[0]=O;for(let s=0;s<O;++s){let f=_[s];for(let m=0;m<l;++m){let T=f[m],u=(r+s*o+m)*4;e[u]=T.x,e[u+1]=1-T.y,e[u+2]=T.z??0,e[u+3]=T.visibility??1}let S=H(e,s,Z,o,r),R=(r+s*o+n.BODY_CENTER)*4;e[R]=S[0],e[R+1]=S[1],e[R+2]=S[2],e[R+3]=S[3];let a=H(e,s,ee,o,r),x=(r+s*o+n.LEFT_HAND_CENTER)*4;e[x]=a[0],e[x+1]=a[1],e[x+2]=a[2],e[x+3]=a[3];let C=H(e,s,te,o,r),P=(r+s*o+n.RIGHT_HAND_CENTER)*4;e[P]=C[0],e[P+1]=C[1],e[P+2]=C[2],e[P+3]=C[3];let N=H(e,s,ne,o,r),L=(r+s*o+n.LEFT_FOOT_CENTER)*4;e[L]=N[0],e[L+1]=N[1],e[L+2]=N[2],e[L+3]=N[3];let D=H(e,s,se,o,r),p=(r+s*o+n.RIGHT_FOOT_CENTER)*4;e[p]=D[0],e[p+1]=D[1],e[p+2]=D[2],e[p+3]=D[3];let E=H(e,s,ie,o,r),t=(r+s*o+n.TORSO_CENTER)*4;e[t]=E[0],e[t+1]=E[1],e[t+2]=E[2],e[t+3]=E[3]}I.state.nPoses=O}function _e(I,_){let{maskShader:e,maxPoses:O}=I;if(!_||_.length===0)return e.clear();for(let s=0;s<_.length;++s){let f=_[s];e.updateTextures({u_mask:f.getAsWebGLTexture()}),e.updateUniforms({u_poseIndex:(s+1)/O}),e.step({skipClear:s>0}),f.close()}}function Te(I){let{textureName:_,options:{history:e,...O}={}}=I,s={...oe,...O},f=B({...s,textureName:_}),S=s.maxPoses*o+r,R=Math.ceil(S/F);return function(a,x){let{injectGLSL:C,emit:P,updateTexture:N}=x,L=y.get(f),D=L?.landmarks.data??new Float32Array(F*R*4),p=L?.mediapipeCanvas??new OffscreenCanvas(1,1),E=L?.maskShader??(()=>{let i=new U(re,{canvas:p});return i.initializeTexture("u_mask",w),i.initializeUniform("u_poseIndex","float",0),i})(),t,m=!1,T=-1,u=[];function G(i){if(!t)return;let{nPoses:d}=t.state,k=d*o+r,M=Math.ceil(k/F),A=e?i:void 0;N("u_poseLandmarksTex",{data:t.landmarks.data,width:F,height:M,isPartial:!0},A),N("u_poseMask",t.maskShader,A),a.updateUniforms({u_nPoses:d},{allowMissing:!0})}function v(){e?(G(u.length>0?u:T),u=[]):G(T),P("pose:result",t.state.result)}async function V(){t=await Y(f,y,ae,async()=>{let[i,{PoseLandmarker:d}]=await Promise.all([z(),import("@mediapipe/tasks-vision")]);if(m)return;let k=await d.createFromOptions(i,{baseOptions:{modelAssetPath:s.modelPath,delegate:"GPU"},canvas:p,runningMode:"VIDEO",numPoses:s.maxPoses,minPoseDetectionConfidence:s.minPoseDetectionConfidence,minPosePresenceConfidence:s.minPosePresenceConfidence,minTrackingConfidence:s.minTrackingConfidence,outputSegmentationMasks:!0});if(m){k.close(),E.destroy();return}return{landmarker:k,mediapipeCanvas:p,maskShader:E,subscribers:new Map,maxPoses:s.maxPoses,state:{nCalls:0,runningMode:"VIDEO",source:null,videoTime:-1,resultTimestamp:0,result:null,pending:Promise.resolve(),nPoses:0},landmarks:{data:D,textureHeight:R}}}),!(!t||m)&&(E!==t.maskShader&&E.destroy(),t.subscribers.set(v,!1))}let K=V();a.on("_init",()=>{a.initializeUniform("u_maxPoses","int",s.maxPoses,{allowMissing:!0}),a.initializeUniform("u_nPoses","int",0,{allowMissing:!0}),a.initializeTexture("u_poseLandmarksTex",{data:D,width:F,height:R},{internalFormat:"RGBA32F",type:"FLOAT",minFilter:"NEAREST",magFilter:"NEAREST",history:e}),a.initializeTexture("u_poseMask",E,{minFilter:"NEAREST",magFilter:"NEAREST",history:e}),K.then(()=>{m||!t||P("pose:ready")})});function $(i){t&&(e&&(T=(T+1)%(e+1),G(T),u.push(T)),t.subscribers.set(v,!0),X(i))}a.on("initializeTexture",(i,d)=>{i===_&&b(d)&&$(d)}),a.on("updateTextures",i=>{let d=i[_];b(d)&&$(d)});async function X(i){let d=performance.now();if(await K,!t)return;let k=++t.state.nCalls;t.state.pending=t.state.pending.then(async()=>{if(!t||k!==t.state.nCalls)return;let M=i instanceof HTMLVideoElement?"VIDEO":"IMAGE";t.state.runningMode!==M&&(t.state.runningMode=M,await t.landmarker.setOptions({runningMode:M}));let A=!1;if(i!==t.state.source?(t.state.source=i,t.state.videoTime=i instanceof HTMLVideoElement?i.currentTime:-1,A=!0):i instanceof HTMLVideoElement?i.currentTime!==t.state.videoTime&&(t.state.videoTime=i.currentTime,A=!0):i instanceof HTMLImageElement||d-t.state.resultTimestamp>2&&(A=!0),A){let c;if(i instanceof HTMLVideoElement){if(i.videoWidth===0||i.videoHeight===0||i.readyState<2)return;c=t.landmarker.detectForVideo(i,d)}else{if(i.width===0||i.height===0)return;c=t.landmarker.detect(i)}if(c){t.state.resultTimestamp=d,t.state.result=c,de(t,c.landmarks),_e(t,c.segmentationMasks);for(let[h,J]of t.subscribers.entries())J&&(h(),t.subscribers.set(h,!1))}}else if(t.state.result)for(let[c,h]of t.subscribers.entries())h&&(c(),t.subscribers.set(c,!1))}),await t.state.pending}a.on("destroy",()=>{m=!0,t&&(t.subscribers.delete(v),t.subscribers.size===0&&(t.landmarker.close(),t.maskShader.destroy(),y.delete(f))),t=void 0});let{fn:g,historyParams:q}=W(e),j=e?`int layer = (u_poseMaskFrameOffset - framesAgo + ${e+1}) % ${e+1};
...
${g("vec4","poseLandmark","int poseIndex, int landmarkIndex",`int i = ${r} + poseIndex * ${o} + landmarkIndex;
	int x = i % ${F};
	int y = i / ${F};${e?`
	int layer = (u_poseLandmarksTexFrameOffset - framesAgo + ${e+1}) % ${e+1};
	return texelFetch(u_poseLandmarksTex, ivec3(x, y, layer), 0);`:`
	return texelFetch(u_poseLandmarksTex, ivec2(x, y), 0);`}`)}
${g("vec2","poseAt","vec2 pos",`${j}
	float poseIndex = floor(mask.b * float(u_maxPoses) + 0.5) - 1.0;
	return vec2(mask.g, poseIndex);`)}
${g("float","inPose","vec2 pos",`vec2 pose = poseAt(pos${q}); return step(0.0, pose.y) * pose.x;`)}`)}}var fe=Te;export{fe as default};
