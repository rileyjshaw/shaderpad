...
}`,C=new Map;function te(L,_){let e=L.landmarks.data,p=_.length;e[0]=p;for(let s=0;s<p;++s){let T=_[s];for(let d=0;d<m;++d){let x=T[d],N=(a+s*i+d)*4;e[N]=x.x,e[N+1]=1-x.y,e[N+2]=x.z??0,e[N+3]=x.visibility??1}let D=P(e,s,X,i,a),c=(a+s*i+n.BODY_CENTER)*4;e[c]=D[0],e[c+1]=D[1],e[c+2]=D[2],e[c+3]=D[3];let r=P(e,s,V,i,a),F=(a+s*i+n.LEFT_HAND_CENTER)*4;e[F]=r[0],e[F+1]=r[1],e[F+2]=r[2],e[F+3]=r[3];let k=P(e,s,q,i,a),R=(a+s*i+n.RIGHT_HAND_CENTER)*4;e[R]=k[0],e[R+1]=k[1],e[R+2]=k[2],e[R+3]=k[3];let u=P(e,s,j,i,a),I=(a+s*i+n.LEFT_FOOT_CENTER)*4;e[I]=u[0],e[I+1]=u[1],e[I+2]=u[2],e[I+3]=u[3];let l=P(e,s,J,i,a),f=(a+s*i+n.RIGHT_FOOT_CENTER)*4;e[f]=l[0],e[f+1]=l[1],e[f+2]=l[2],e[f+3]=l[3];let t=P(e,s,Q,i,a),O=(a+s*i+n.TORSO_CENTER)*4;e[O]=t[0],e[O+1]=t[1],e[O+2]=t[2],e[O+3]=t[3]}L.state.nPoses=p}function ne(L,_){let{maskShader:e,maxPoses:p}=L;if(!_||_.length===0)return e.clear();for(let s=0;s<_.length;++s){let T=_[s];e.updateTextures({u_mask:T.getAsWebGLTexture()}),e.updateUniforms({u_poseIndex:(s+1)/p}),e.draw({skipClear:s>0}),T.close()}}function se(L){let{textureName:_,options:{history:e,...p}={}}=L,s={...Z,...p},T=U({...s,textureName:_}),D=s.maxPoses*i+a,c=Math.ceil(D/A);return function(r,F){let{injectGLSL:k,emitHook:R}=F,u=C.get(T),I=u?.landmarks.data??new Float32Array(A*c*4),l=u?.mediapipeCanvas??new OffscreenCanvas(1,1),f=u?.maskShader??(()=>{let o=new $(ee,{canvas:l});return o.initializeTexture("u_mask",b),o.initializeUniform("u_poseIndex","float",0),o})(),t=null,O=!1;function d(){if(!t)return;let{nPoses:o}=t.state,E=o*i+a,H=Math.ceil(E/A);r.updateTextures({u_poseLandmarksTex:{data:t.landmarks.data,width:A,height:H,isPartial:!0},u_poseMask:f},{skipHistoryWrite:O}),r.updateUniforms({u_nPoses:o}),R("pose:result",t.state.result)}async function x(){if(C.has(T))t=C.get(T);else{let[o,{PoseLandmarker:E}]=await Promise.all([w(),import("@mediapipe/tasks-vision")]);t={landmarker:await E.createFromOptions(o,{baseOptions:{modelAssetPath:s.modelPath,delegate:"GPU"},canvas:l,runningMode:"VIDEO",numPoses:s.maxPoses,minPoseDetectionConfidence:s.minPoseDetectionConfidence,minPosePresenceConfidence:s.minPosePresenceConfidence,minTrackingConfidence:s.minTrackingConfidence,outputSegmentationMasks:!0}),mediapipeCanvas:l,maskShader:f,subscribers:new Map,maxPoses:s.maxPoses,state:{runningMode:"VIDEO",source:null,videoTime:-1,resultTimestamp:0,result:null,pending:Promise.resolve(),nPoses:0},landmarks:{data:I,textureHeight:c}},C.set(T,t)}t.subscribers.set(d,!1)}let N=x();r.on("init",()=>{r.initializeUniform("u_maxPoses","int",s.maxPoses),r.initializeUniform("u_nPoses","int",0),r.initializeTexture("u_poseLandmarksTex",{data:I,width:A,height:c},{internalFormat:"RGBA32F",type:"FLOAT",minFilter:"NEAREST",magFilter:"NEAREST",history:e}),r.initializeTexture("u_poseMask",f,{minFilter:"NEAREST",magFilter:"NEAREST",history:e}),N.then(()=>R("pose:ready"))}),r.on("initializeTexture",(o,E)=>{o===_&&G(E)&&y(E)}),r.on("updateTextures",(o,E)=>{let H=o[_];G(H)&&(O=E?.skipHistoryWrite??!1,y(H))});let v=0;async function y(o){let E=performance.now(),H=++v;await N,t&&(t.state.pending=t.state.pending.then(async()=>{if(H!==v||!t)return;let h=o instanceof HTMLVideoElement?"VIDEO":"IMAGE";t.state.runningMode!==h&&(t.state.runningMode=h,await t.landmarker.setOptions({runningMode:h}));let g=!1;if(o!==t.state.source?(t.state.source=o,t.state.videoTime=-1,g=!0):o instanceof HTMLVideoElement?o.currentTime!==t.state.videoTime&&(t.state.videoTime=o.currentTime,g=!0):o instanceof HTMLImageElement||E-t.state.resultTimestamp>2&&(g=!0),g){let S;if(o instanceof HTMLVideoElement){if(o.videoWidth===0||o.videoHeight===0||o.readyState<2)return;S=t.landmarker.detectForVideo(o,E)}else{if(o.width===0||o.height===0)return;S=t.landmarker.detect(o)}if(S){t.state.resultTimestamp=E,t.state.result=S,te(t,S.landmarks),ne(t,S.segmentationMasks);for(let K of t.subscribers.keys())K(),t.subscribers.set(K,!0)}}else t.state.result&&!t.subscribers.get(d)&&(d(),t.subscribers.set(d,!0))}),await t.state.pending)}r.on("destroy",()=>{t&&(t.subscribers.delete(d),t.subscribers.size===0&&(t.landmarker.close(),t.maskShader.destroy(),C.delete(T))),t=null});let{fn:M,historyParams:W}=B(e),Y=e?`int layer = (u_poseMaskFrameOffset - framesAgo + ${e}) % ${e};
...
${M("vec4","poseLandmark","int poseIndex, int landmarkIndex",`int i = ${a} + poseIndex * ${i} + landmarkIndex;
	int x = i % ${A};
	int y = i / ${A};${e?`
	int layer = (u_poseLandmarksTexFrameOffset - framesAgo + ${e}) % ${e};
...
${M("float","inPose","vec2 pos",`vec2 pose = poseAt(pos${W}); return step(0.0, pose.y) * pose.x;`)}`)}}var Ee=se;export{Ee as default};
