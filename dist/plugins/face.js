...
out vec4 outColor;
void main() { outColor = u_color; }`,S=478,ge=2,F=S+ge,x=512,h=1,ae=[336,296,334,293,300,276,283,282,295,285],re=[362,398,384,385,386,387,388,466,263,249,390,373,374,380,381,382],se=[70,63,105,66,107,55,65,52,53,46],ie=[33,246,161,160,159,158,157,173,133,155,154,153,145,144,163,7],oe=[61,185,40,39,37,0,267,269,270,409,291,375,321,405,314,17,84,181,91,146],B=[78,191,80,81,82,13,312,311,310,415,308,324,318,402,317,14,87,178,88,95],Re=Array.from({length:S},(a,e)=>e),k={LEFT_EYEBROW:ae,LEFT_EYE:re,LEFT_EYE_CENTER:473,RIGHT_EYEBROW:se,RIGHT_EYE:ie,RIGHT_EYE_CENTER:468,NOSE_TIP:4,OUTER_MOUTH:oe,INNER_MOUTH:B,FACE_CENTER:S,MOUTH_CENTER:S+1},ce=["BACKGROUND","LEFT_EYEBROW","RIGHT_EYEBROW","LEFT_EYE","RIGHT_EYE","OUTER_MOUTH","INNER_MOUTH"],me=ce.length-1,M=Object.fromEntries(ce.map((a,e)=>[a,e/me])),ne=.5/me,Me={modelPath:"https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/latest/face_landmarker.task",maxFaces:1,minFaceDetectionConfidence:.5,minFacePresenceConfidence:.5,minTrackingConfidence:.5,outputFaceBlendshapes:!1,outputFacialTransformationMatrixes:!1};function O(a){let e=[];for(let t=1;t<a.length-1;++t)e.push(a[0],a[t],a[t+1]);return e}var T=null;function he(a){if(!T){let e=a.FACE_LANDMARKS_TESSELATION,t=[];for(let n=0;n<e.length-2;n+=3)t.push(e[n].start,e[n+1].start,e[n+2].start);let i=a.FACE_LANDMARKS_FACE_OVAL.map(({start:n})=>n);T=Object.fromEntries(Object.entries({LEFT_EYEBROW:O(ae),RIGHT_EYEBROW:O(se),LEFT_EYE:O(re),RIGHT_EYE:O(ie),OUTER_MOUTH:O(oe),INNER_MOUTH:O(B),TESSELATION:t,OVAL:O(i)}).map(([n,c])=>[n,{triangles:c,vertices:new Float32Array(c.length*2)}]))}}var C=new Map;function Le(a){let e=a.getContext("webgl2",{antialias:!1,preserveDrawingBuffer:!0}),t=e.createShader(e.VERTEX_SHADER);e.shaderSource(t,Ae),e.compileShader(t);let i=e.createShader(e.FRAGMENT_SHADER);e.shaderSource(i,Fe),e.compileShader(i);let n=e.createProgram();e.attachShader(n,t),e.attachShader(n,i),e.linkProgram(n),e.deleteShader(t),e.deleteShader(i);let c=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,c);let d=e.getAttribLocation(n,"a_pos");e.enableVertexAttribArray(d),e.vertexAttribPointer(d,2,e.FLOAT,!1,0,0);let l=e.getUniformLocation(n,"u_color");return e.useProgram(n),e.enable(e.BLEND),e.blendEquation(e.MAX),{canvas:a,gl:e,program:n,positionBuffer:c,colorLocation:l}}function R(a,e,t,i,n,c,d){let{triangles:l,vertices:o}=t,{gl:m,colorLocation:u}=a;for(let f=0;f<l.length;++f){let p=(h+i*F+l[f])*4;o[f*2]=e[p],o[f*2+1]=e[p+1]}m.bufferData(m.ARRAY_BUFFER,o,m.DYNAMIC_DRAW),m.uniform4f(u,n,c,d,1),m.drawArrays(m.TRIANGLES,0,l.length)}function xe(a,e){let t=a.landmarks.data,i=e.length;t[0]=i;for(let n=0;n<i;++n){let c=e[n];for(let o=0;o<S;++o){let m=c[o],u=(h+n*F+o)*4;t[u]=m.x,t[u+1]=1-m.y,t[u+2]=m.z??0,t[u+3]=m.visibility??1}let d=w(t,n,Re,F,h);t.set(d,(h+n*F+k.FACE_CENTER)*4);let l=w(t,n,B,F,1);t.set(l,(h+n*F+k.MOUTH_CENTER)*4)}a.state.nFaces=i}function Oe(a,e,t){if(!T)return;let{mask:i,maxFaces:n,landmarks:c,state:{nFaces:d}}=a,{gl:l,canvas:o}=i,{data:m}=c;(o.width!==e||o.height!==t)&&(o.width=e,o.height=t),l.viewport(0,0,o.width,o.height),l.clearColor(0,0,0,0),l.clear(l.COLOR_BUFFER_BIT);for(let u=0;u<d;++u){let f=(u+1)/n;R(i,m,T.TESSELATION,u,0,.5,f),R(i,m,T.OVAL,u,0,1,f),R(i,m,T.LEFT_EYEBROW,u,M.LEFT_EYEBROW,0,f),R(i,m,T.RIGHT_EYEBROW,u,M.RIGHT_EYEBROW,0,f),R(i,m,T.LEFT_EYE,u,M.LEFT_EYE,0,f),R(i,m,T.RIGHT_EYE,u,M.RIGHT_EYE,0,f),R(i,m,T.OUTER_MOUTH,u,M.OUTER_MOUTH,0,f),R(i,m,T.INNER_MOUTH,u,M.INNER_MOUTH,0,f)}}function ke(a){let{textureName:e,options:{history:t,...i}={}}=a,n={...Me,...i},c=Q({...n,textureName:e}),d=n.maxFaces*F+h,l=Math.ceil(d/x);return function(o,m){let{injectGLSL:u,emitHook:f}=m,p=C.get(c),v=p?.landmarks.data??new Float32Array(x*l*4),b=p?.mask.canvas??new OffscreenCanvas(1,1),r=null,G=!1;function N(){if(!r)return;let s=r.state.nFaces,_=s*F+h,A=Math.ceil(_/x);o.updateTextures({u_faceLandmarksTex:{data:r.landmarks.data,width:x,height:A,isPartial:!0},u_faceMask:r.mask.canvas},{skipHistoryWrite:G}),o.updateUniforms({u_nFaces:s}),f("face:result",r.state.result)}async function le(){if(C.has(c))r=C.get(c);else{let[s,{FaceLandmarker:_}]=await Promise.all([ee(),import("@mediapipe/tasks-vision")]),A=new OffscreenCanvas(1,1);r={landmarker:await _.createFromOptions(s,{baseOptions:{modelAssetPath:n.modelPath,delegate:"GPU"},canvas:A,runningMode:"VIDEO",numFaces:n.maxFaces,minFaceDetectionConfidence:n.minFaceDetectionConfidence,minFacePresenceConfidence:n.minFacePresenceConfidence,minTrackingConfidence:n.minTrackingConfidence,outputFaceBlendshapes:n.outputFaceBlendshapes,outputFacialTransformationMatrixes:n.outputFacialTransformationMatrixes}),mediapipeCanvas:A,mask:Le(b),subscribers:new Map,maxFaces:n.maxFaces,state:{runningMode:"VIDEO",source:null,videoTime:-1,resultTimestamp:0,result:null,pending:Promise.resolve(),nFaces:0},landmarks:{data:v,textureHeight:l}},he(_),C.set(c,r)}r.subscribers.set(N,!1)}let W=le(),V=0;async function K(s){let _=performance.now(),A=++V;await W,r&&(r.state.pending=r.state.pending.then(async()=>{if(A!==V||!r)return;let y=s instanceof HTMLVideoElement?"VIDEO":"IMAGE";r.state.runningMode!==y&&(r.state.runningMode=y,await r.landmarker.setOptions({runningMode:y}));let $=!1;if(s!==r.state.source?(r.state.source=s,r.state.videoTime=-1,$=!0):s instanceof HTMLVideoElement?s.currentTime!==r.state.videoTime&&(r.state.videoTime=s.currentTime,$=!0):s instanceof HTMLImageElement||_-r.state.resultTimestamp>2&&($=!0),$){let I,H,U;if(s instanceof HTMLVideoElement){if(s.videoWidth===0||s.videoHeight===0||s.readyState<2)return;H=s.videoWidth,U=s.videoHeight,I=r.landmarker.detectForVideo(s,_)}else{if(s.width===0||s.height===0)return;H=s.width,U=s.height,I=r.landmarker.detect(s)}if(I){r.state.resultTimestamp=_,r.state.result=I,xe(r,I.faceLandmarks),Oe(r,H,U);for(let q of r.subscribers.keys())q(),r.subscribers.set(q,!0)}}else r.state.result&&!r.subscribers.get(N)&&(N(),r.subscribers.set(N,!0))}),await r.state.pending)}o.on("init",()=>{o.initializeUniform("u_maxFaces","int",n.maxFaces),o.initializeUniform("u_nFaces","int",0),o.initializeTexture("u_faceLandmarksTex",{data:v,width:x,height:l},{internalFormat:"RGBA32F",type:"FLOAT",minFilter:"NEAREST",magFilter:"NEAREST",history:t}),o.initializeTexture("u_faceMask",b,{minFilter:"NEAREST",magFilter:"NEAREST",history:t}),W.then(()=>f("face:ready"))}),o.on("initializeTexture",(s,_)=>{s===e&&Y(_)&&K(_)}),o.on("updateTextures",(s,_)=>{let A=s[e];Y(A)&&(G=_?.skipHistoryWrite??!1,K(A))}),o.on("destroy",()=>{r&&(r.subscribers.delete(N),r.subscribers.size===0&&(r.landmarker.close(),r.mask.gl.deleteProgram(r.mask.program),r.mask.gl.deleteBuffer(r.mask.positionBuffer),C.delete(c))),r=null});let{fn:E,historyParams:g}=te(t),z=t?"_sampleFaceMask(pos, framesAgo)":"texture(u_faceMask, pos)",L=(s,_=s)=>`vec4 mask = ${z};
	...
	float faceIndex = floor(mask.b * float(u_maxFaces) + 0.5) - 1.0;
	return mask.g > ${s.toFixed(2)} ? vec2(1.0, faceIndex) : vec2(0.0, -1.0);`,X=(s,_)=>`vec2 left = ${s}(pos${g});
	return left.x > 0.0 ? left : ${_}(pos${g});`;u(`
...
${E("vec4","faceLandmark","int faceIndex, int landmarkIndex",`int i = ${h} + faceIndex * ${F} + landmarkIndex;
	int x = i % ${x};
	int y = i / ${x};${t?`
	int layer = (u_faceLandmarksTexFrameOffset - framesAgo + ${t}) % ${t};
...
`:""}
${E("vec2","leftEyebrowAt","vec2 pos",L("LEFT_EYEBROW"))}
${E("vec2","rightEyebrowAt","vec2 pos",L("RIGHT_EYEBROW"))}
${E("vec2","leftEyeAt","vec2 pos",L("LEFT_EYE"))}
${E("vec2","rightEyeAt","vec2 pos",L("RIGHT_EYE"))}
${E("vec2","lipsAt","vec2 pos",L("OUTER_MOUTH"))}
${E("vec2","outerMouthAt","vec2 pos",L("OUTER_MOUTH","INNER_MOUTH"))}
${E("vec2","innerMouthAt","vec2 pos",L("INNER_MOUTH"))}
...
${E("float","inEyebrow","vec2 pos",`return eyebrowAt(pos${g}).x;`)}
${E("float","inEye","vec2 pos",`return eyeAt(pos${g}).x;`)}
${E("float","inOuterMouth","vec2 pos",`return outerMouthAt(pos${g}).x;`)}
${E("float","inInnerMouth","vec2 pos",`return innerMouthAt(pos${g}).x;`)}
${E("float","inLips","vec2 pos",`return lipsAt(pos${g}).x;`)}
${E("float","inFace","vec2 pos",`return faceAt(pos${g}).x;`)}`)}}var ve=ke;
